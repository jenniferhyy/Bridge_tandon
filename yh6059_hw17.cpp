...
void RBT<T>::insert(const T &toInsert, RBTNode<T> *&point, RBTNode<T> *parent) {
    if (point == nullptr) {               // leaf location is found so insert node
        ...
        while (curr_node != nullptr && curr_node->parent != nullptr && getColor(curr_node->parent) == RED) {
            RBTNode<T> *parent = curr_node->parent;
            RBTNode<T> *grandparent = parent->parent;
            RBTNode<T> *uncle = (grandparent->left == parent) ? grandparent->right : grandparent->left;

            if (getColor(uncle) == RED) {
                swapColor(parent); // Parent to black
                swapColor(uncle); // Uncle to black
                swapColor(grandparent); // Grandparent to red
                curr_node = grandparent; // Move up to grandparent
            } else {
                ...
                if (grandparent->left == parent) {
                    ...
                    if (parent->left == curr_node) {
                        swapColor(parent); // Parent to black
                        swapColor(grandparent); // Grandparent to red
                        singleCR(grandparent); // Right rotation
                    }

                    else {
                        curr_node = parent; // Move up to parent for double rotation
                        doubleCR(grandparent); // Left rotation on parent, then right on grandparent
                        swapColor(curr_node); // New parent (former parent) to black
                        swapColor(grandparent); // Grandparent to red
                    }
                }
                // Parent is right child of grandparent
                else {
                    // Case 3: Outer child (curr_node is right child of parent)
                    if (parent->right == curr_node) {
                        swapColor(parent); // Parent to black
                        swapColor(grandparent); // Grandparent to red
                        singleCCR(grandparent); // Left rotation
                    }

                    else {
                        curr_node = parent; // Move up to parent for double rotation
                        doubleCCR(grandparent); // Right rotation on parent, then left on grandparent
                        swapColor(curr_node); // New parent (former parent) to black
                        swapColor(grandparent); // Grandparent to red
                    }
                }
